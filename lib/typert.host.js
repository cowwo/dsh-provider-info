...
const balanceResultSchema = z.object({
	...
	// Command Code（GOAT 等）：月度 credits 剩余（remaining/total=周窗cap×2，total 不可推导时为 null）
	monthly: z.object({
		remaining: z.number().nullable().optional(),
		total: z.number().nullable().optional(),
		...
		services: [
			{
				...
				members: [
					...
					},
					{
						kind: 'method',
						name: 'settings',
						signature: 'async settings(request: { op: string; patch?: object }): Promise<{ settings: { hoverRefresh?: boolean | null; autoRefreshOn?: boolean | null; autoRefreshMin?: number | null } }>',
