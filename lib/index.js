...
const SETTINGS_DEFAULTS = { hoverRefresh: true, autoRefreshOn: false, autoRefreshMin: 5, showMore: true, fontSize: 'middle', language: 'system' }
const settingPath = () => {
...
const clampSettings = (s) => {
	...
	if (s && typeof s === 'object') {
		...
		// 默认开（与 hoverRefresh 同样的口径：只有显式 false 才算关，脏数据/缺字段一律回落默认开）。
		// 打开时余量额外显示金额、浮层显示重置倒计时与到期剩余天数。
		out.showMore = s.showMore !== false
