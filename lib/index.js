...
async function queryCommandCode(base, key) {
	...
	// 订阅计费周期：只有 subscriptions 接口给了结束时间才算数（opencode 不提供，故它没有「到期」）。
	let period = null
	if (!subRes.httpError && subRes.body && subRes.body.data) {
		const end = typeof subRes.body.data.currentPeriodEnd === 'string' ? subRes.body.data.currentPeriodEnd : null
		if (end) {
			const ms = Date.parse(end)
			period = { end, daysLeft: ms === ms ? Math.max(0, Math.ceil((ms - Date.now()) / 86400000)) : null }
		}
	}
