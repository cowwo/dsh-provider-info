...
//   GET /alpha/billing/subscriptions  → planId（用于映射官方月度额度上限，映射不到时只显示剩余）
// 归一化成与 OpenCode Go 一致的 window 结构（rolling=5小时、weekly=7天）。
const COMMANDCODE_MONTHLY_LIMITS = {
...
async function queryCommandCode(base, key) {
	...
	const creditsRes = await requestJson(origin + '/alpha/billing/credits', authHeaders)
	...
	for (const [winKey, label, outKey] of [['fiveHour', '5小时', 'rolling'], ['weekly', '7天', 'weekly']]) {
		...
		const pct = used === null ? null : Math.round((used / cap) * 1000) / 10
		windows.push({
	...
	// 月度额度剩余：credits 余额里的月度池剩余 + 额外购买（购买部分不受窗口限制，一并计入“还能用多少”）。
	const monthlyRemaining = (toNum(credits.monthlyCredits) || 0) + (toNum(credits.purchasedCredits) || 0)
	let monthly = null
	if (credits.monthlyCredits != null || credits.purchasedCredits != null) {
		let planId = null
		const subRes = await requestJson(origin + '/alpha/billing/subscriptions', authHeaders)
		if (!subRes.httpError && subRes.body && subRes.body.data && typeof subRes.body.data.planId === 'string') {
			planId = subRes.body.data.planId
		}
		const total = planId && COMMANDCODE_MONTHLY_LIMITS[planId] != null ? COMMANDCODE_MONTHLY_LIMITS[planId] : null
		monthly = { remaining: Math.round(monthlyRemaining * 100) / 100, total: total == null ? null : total, plan: planId || null, currency: 'USD' }
	}
