...
### Added
- **OpenCode Go 也有「到期」了**：月窗的重置时间就是订阅计费周期的终点，浮层与设置页表格因此多出一行/一格「到期」（如 `2026-09-24`，开「显示更多信息」后附带「剩 10 天」）。
  - 接口本身**没有**独立的订阅字段（实测 `/v1/usage` 只回 `rolling / weekly / monthly` 的 `status / percent / resetsAt`；`/v1/subscription`、`/v1/me`、`/v1/billing`、`/v1/plan` 均 404），所以这里由月窗 `resetsAt` 推导：它不是日历锚点（周窗锚在周一 00:00Z 整点；月窗锚在非整点的账号事件 ≈ 订阅起算日 + 1 个月，间隔 12s 连续采样不随时钟漂移）——缺的是"独立字段"，不是"周期终点"。
  - 「月」行照旧带自己的重置倒计时：`windowValue` 里"倒计时与「到期」是同一时刻就跳过"的去重规则**已移除**，`5小时 / 周 / 月` 三行现在一律显示倒计时（Command Code 的月行同样恢复）。「到期」行仍单独给日期 / 剩余天数——两行各答一半（月行＝还有多久，到期行＝哪天），不再互相顶掉。

### Changed
- `lib/index.js` 抽出共享的 `makePeriod(end)`（`end` + `daysLeft`），Command Code 与 OpenCode Go 共用同一份「到期」形状；`queryOpencodeGo` 新增返回 `period`。`providerBadge/balance` 的整体结构与 `typert` 严格 schema 无需改动——`period` 本来就是可选维度（缺失恒为 `null`）。
- 渲染端 `client/client.js`：`windowValue` 的倒计时去重逻辑移除（`opts.period` 参数随之不再需要），窗口行的倒计时规则回到"每个窗口都显示自己的"；`tools/verify-format.mjs` 同步断言（新增"月行确实带倒计时"两条，og-01 与 cmd-01 各一）。
- **浮层余量区块的窗口行改成四列网格对齐**（标签 | 比例 | 金额 | 尾列）：原来每行是一整串右对齐文本，`4.00%` 与 `77.00%` 宽度不同会把后面的金额、倒计时推得参差不齐；现在窗口行共用一个 grid 容器，列宽由最宽的一格决定，各列右边缘自动对齐。为此把 `windowValue()` 拆出分列版 `windowCells()`（`pct` / `money` / `tail`），`windowValue()` 仍负责拼成一行，供设置页表格与降级路径使用（同一份来源，两处不会不一致）。
- 金额去掉外层括号、段间统一用 `·`：`4.00% · $0.48/$12.00 · 3h31m`（浮层分列显示、表格一行显示，字符完全相同）。
  - 点由分列函数挂在前一段末尾（`比例 ·` / `金额 ·`）而不是下一段的开头：右对齐的单元格里结尾字符天然对齐成一列，所以 `4.00%` 与 `77.00%`、`$0.48/$12.00` 与 `$46.20/$60.00` 的点都在同一竖线上；后面没有内容（如关掉「显示更多信息」）时不挂点，不会留下孤零零的 `·`。
- **「到期」行不参与该网格**：它按浮层普通的「标签 + 右对齐文本」行渲染（`2026-09-24 · 剩 9 天`），只有窗口行对齐。
- **没有重置时间的窗口，尾列补占位 `—`**：Command Code 在某个窗口还没被任何请求打开时，接口把 `resetAt` 写成 `0`（不是时间戳）——原先插件把它当 `1970-01-01`，客户端算出负倒计时后这个格子就空着，看起来跟 OpenCode Go（接口每行都给 `resetsAt`，永远有真实倒计时）不一致。现在 host 把 `0` / 缺失一律视作「没有重置时间」，客户端在该列显示占位 `—`，窗口行四列永远完整。占位文案是单个 i18n 键 `noReset`，想换成「未开始 / no window」只改一处。
- 补「跨厂商一致性」回归（`tools/verify-format.mjs`）：把 `client.js` 浮层那段**真实源码**抽出来、用最小 DOM 替身跑 `balanceRows()`，断言 ① og-01 与 cmd-01 的**结构指纹完全一致**（列位 `auto,2,3,4`、分隔点位置、「到期」是独立普通行），② 比例列与金额列都以 ` ·` 结尾、尾列不带前导点，③ 关掉「显示更多信息」后窗口行只剩比例且不带点。夹具里的 `resetsAt` / 到期时间改为**相对当前时间**生成（字段结构照抄实测样本），避免断言随日期失效。
- 文档同步：`CONTEXT.md` 的「到期」「配额窗口」「支持查询」三处术语、`README.md` / `README.zh.md` 的 OpenCode Go 条目（原先写的"接口不提供订阅到期、所以没有到期行"只对"独立字段"成立，已改写）。

### Notes
- 客户端除了"倒计时去重"这条规则，还改了余量区块的排版（四列网格 + 金额去括号 + 段间 `·`）；其余渲染逻辑（谁有数据画谁、窗口按真实时长排序、到期行独立）本来就在，host 补上 `period` 后自动生效。
- 金额仍是近似：OpenCode Go 官方月额度是**按模型**的（$15 / $30 / $60 …），而 `/v1/usage` 只回账号级百分比、不吃 `model` 参数，故「已用 $/总额 $」继续按 `OPENCODE_LIMITS_USD = { rolling: 12, weekly: 30, monthly: 60 }` 折算，对在用低档模型的账号会偏高。
