...
- **Quota / balance section**: for providers that support it, the hover panel also shows the provider's remaining quota — DeepSeek account balance, or OpenCode Go's 5-hour / weekly / monthly usage limits with reset countdowns. Recognized-but-unsupported providers show `暂不支持该供应商查询`; missing keys or query failures show `未配置 API Key` / `查询失败`.
  ...
  - Command Code (GOAT / Pro / Max plans, e.g. `https://api.commandcode.ai/provider/v1`): remaining monthly credits (total pool derived as weekly cap × 2, with billing-period expiry) plus 5-hour / weekly window usage with two-decimal percentages and reset countdowns (the same read-only internal endpoints the official CLI uses).
  - Providers that can't be queried (including unrecognized ones) still show a quota row in the hover panel: `当前暂不支持查询当前提供商`; missing keys or query failures show `未配置 API Key` / `查询失败`.
