...
- **余量区块**：对支持查询的厂商，悬浮浮层还会展示当前提供商的余量。**两类提供商共用同一套呈现规则**：
  ...
  - **套餐型**（如 **OpenCode Go**、**Command Code**）：按窗口逐行显示，行名是窗口周期名 `5小时 / 周 / 月`，行内容是 `已用%（已用 $/总额 $）· 重置倒计时`；厂商提供订阅计费周期时，最后再加一行「到期」（如 `2026-10-08 · 剩 27 天`）。
    - **OpenCode Go**：`5小时 / 周 / 月` 三个滚动窗口。接口只回百分比 + 重置时间，金额按官方套餐价常量折算；接口不提供订阅到期信息，所以没有「到期」行。
  ...
  - 不支持查询的提供商（含未识别厂商）浮窗照常显示「当前暂不支持查询当前提供商」；未配置密钥显示「未配置 API Key」；查询失败显示「查询失败」。
