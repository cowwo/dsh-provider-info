...
- 切换模型后，旁边那行小字会自动跟着变。
- **悬停即显**：鼠标移到提供商标签上立刻显示面板（用本地缓存同步渲染，余量未就绪时先显示「刷新中…」，数据回来后在后台补齐），不会出现“悬停没反应、移开后才弹出”；标签热区比视觉略大，短暂擦边不会丢悬停。
- **不碰密钥**：它只显示密钥对应的*环境变量名*，绝不会读取或显示密钥的真实内容。
- **余量区块**：对支持查询的厂商，悬浮浮层还会展示当前提供商的余量——
  - **DeepSeek**：账户余额金额（多币种）。
  - **OpenCode Go**：5小时 / 每周 / 每月 已用百分比 + 重置倒计时 + 折算金额。
  - **Command Code**（GOAT / Pro / Max 等套餐，如 `https://api.commandcode.ai/provider/v1`）：月度剩余 credits（总额度 = 周窗 × 2 动态推导，带周期到期时间）+ 5小时 / 每周 窗口已用百分比（两位小数）、重置倒计时（官方 CLI 同款内部接口，只读）。
