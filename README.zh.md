...
- **余量区块**：对支持查询的厂商，悬浮浮层还会展示当前提供商的余量——
  ...
  - **刷新行为可在「设置 → 提供商余量」调整**：开启*鼠标悬停立即刷新*（默认开）则每次悬停都绕开缓存；或开启*自动刷新*（默认关）并设置间隔（默认 5 分钟，最低 1 分钟），在浮窗打开时定时重新查询。设置持久化于 `$DSH_HOME/dsh-provider-info.json`（权限 0600）。
...
- **鼠标悬停立即刷新**（默认勾选）：每次鼠标移到徽章上就立即重新查询余量，绕开 5 分钟缓存。
- **自动刷新**（默认不勾选）+ **自动刷新间隔(分钟)**（默认 5，最低 1）：浮窗打开时按设定间隔自动重新查询余量。
